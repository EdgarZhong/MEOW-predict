...
| **交付接线 — fit/predict 签名核验（遗留）** | 老师文档只说"会替换数据集路径"、**不严格要求壳层与示例完全一致**——只要能替换路径 + `python meow.py` 跑通 + 结果喂他原封不动的评测器即可。代码侧已确认 `fit/eval` 无藏划分器、训练/测试均全量(`meow/meow.py:111/121`，无降采样)。**遗留待核验**：我们 `MeowEngine.fit/eval/predict` 的调用约定能否让老师顺利替换路径并跑通——尤其 `predict(xdf)` 当前接特征帧，而老师 harness 可能按路径/日期取数；交付前对照 `meow/MEOW金融时序预测2.0.docx` 壳层约定逐字确认。**本会话只做 DL 地基，此项另起会话专办** | 🔜 遗留（另会话） |
